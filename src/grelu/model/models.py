...
class EnformerPretrainedModel(BaseModel):
    ...
    def __init__(
        ...
        if organism == "human":
            n_tasks_default = 5313
        elif organism == "mouse":
            n_tasks_default = 1643

        model = EnformerModel(
            crop_len=crop_len,
            n_tasks=n_tasks_default,
            channels=1536,
            n_transformers=11,
            n_heads=8,
            key_len=64,
        ...
        art = get_artifact(f"{organism}_state_dict", project="enformer", alias="latest")
        with TemporaryDirectory() as d:
            art.download(d)
            state_dict = torch.load(Path(d) / f"{organism}.h5")
